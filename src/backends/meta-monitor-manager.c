                                               ...
                                               MetaMonitorManager    *manager)
{
  ...
  g_assert (g_str_has_prefix (G_OBJECT_TYPE_NAME (manager),
                              "MetaMonitorManager"));
  renderer = g_ascii_strdown (G_OBJECT_TYPE_NAME (manager) +
                              strlen ("MetaMonitorManager"),
                              -1);
                                                   ...
                                                   MetaMonitorManager    *manager)
{
  ...
  if (manager->layout_mode == META_LOGICAL_MONITOR_LAYOUT_MODE_GLOBAL_UI_LOGICAL)
    {
      GList *l;
      int ui_scale = (int) ceilf (max_scale);
      ...
      for (l = logical_monitor_configs; l; l = l->next)
        {
          ...
          logical_monitor_config->layout.width =
            (int) roundf (logical_monitor_config->layout.width * ui_scale);
          logical_monitor_config->layout.height =
            (int) roundf (logical_monitor_config->layout.height * ui_scale);
