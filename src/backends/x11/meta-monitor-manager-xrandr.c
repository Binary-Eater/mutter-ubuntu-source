                                                ...
                                                float                     scale)
{
   ...
   */
  width_mm = (int) ((width / (DPI_FALLBACK * scale)) * 25.4 + 0.5);
  height_mm = (int) ((height / (DPI_FALLBACK * scale)) * 25.4 + 0.5);
      ...
      if (crtc_assignment->mode != NULL)
        {
          ...
          if (have_scaling &&
            ...
            {
              meta_topic (META_DEBUG_X11, "Scalig CRTC %d at %f failed",
  ...
  for (l = manager->monitors; l != NULL; l = l->next)
    {
      ...
      screen_width = MAX (screen_width, (int) (crtc_layout->origin.x +
                          roundf (crtc_layout->size.width * scale)));
      screen_height = MAX (screen_height, (int) (crtc_layout->origin.y +
...
meta_monitor_manager_xrandr_constructed (GObject *object)
{
  ...
  g_signal_connect_object (settings, "x11-scale-mode-changed",
                           G_CALLBACK (scale_mode_changed), manager_xrandr, G_CONNECT_DEFAULT);
